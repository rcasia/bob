...
pub async fn start(
    ...
    if version.version_type == VersionType::Nightly {
            let nightly_version = helpers::version::nightly::get_upstream_nightly(client).await?;

        info!("Looking for nightly updates");

        match helpers::version::nightly::get_local_nightly(config).await {
            Ok(local_nightly) => {
                info!("Found local nightly. Published at: {}", local_nightly.published_at);

                let upstream_nightly = nightly_version.clone();
            ...
            Err(_) => {
                return Err(anyhow!("No local nightly found. Try running `bob install nightly`"));
            }
        };
